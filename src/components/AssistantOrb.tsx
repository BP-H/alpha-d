...
export default function AssistantOrb() {
  ...
  async function handleCommand(text: string) {
    ...
    if (lower.startsWith("/react")) {
      ...
      if (post) { bus.emit("post:react", { id: post.id, emoji }); push({ id: uuid(), role: "assistant", text: `✨ Reacted ${emoji}`, ts: Date.now(), postId: post.id as any }); }
      else { push({ id: uuid(), role: "assistant", text: `⚠️ Drag the orb over a post first.`, ts: Date.now() }); }
    } else if (lower.startsWith("/comment ")) {
      const body = T.slice(9).trim();
      if (post) { bus.emit?.("post:comment", { id: post.id, body }); push({ id: uuid(), role: "assistant", text: `💬 Commented: ${body}`, ts: Date.now(), postId: post.id as any }); }
      else { push({ id: uuid(), role: "assistant", text: `⚠️ Drag onto a post to comment.`, ts: Date.now() }); }
    } else if (lower.startsWith("/world")) {
      bus.emit?.("orb:portal", { x: posRef.current.x, y: posRef.current.y });
      push({ id: uuid(), role: "assistant", text: `🌀 Entering world…`, ts: Date.now() });
    } else if (lower.startsWith("/remix")) {
      if (post) { bus.emit("post:remix", { id: post.id }); push({ id: uuid(), role: "assistant", text: `🎬 Remixing ${post.id}`, ts: Date.now(), postId: post.id as any }); }
      else { push({ id: uuid(), role: "assistant", text: `⚠️ Drag onto a post to remix.`, ts: Date.now() }); }
    } else {
    ...
    <>
      ...
      {open && (
        <div ref={panelRef} style={panelStyle}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(10, 1fr)", gap: 6, padding: "10px 4px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 10, marginTop: 8 }}>
